...
class IconCache private constructor() {
	...
	companion object {
		...
		@JvmStatic fun removeBackground(bitmap:Bitmap?, dest:Int = Color.TRANSPARENT) {
		...
		// 环状扫描
		fun circularScan(pixels:IntArray, width:Int, height:Int, map:MutableMap<Int, Int>, from:Float, to:Float = from + 1) {
			assert(from > to)
